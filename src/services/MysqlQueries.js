...
exports.SELECT_SEARCH_PRODUCT = "SELECT id, codigo_barras, nombre, descripcion, precio_compra, precio_venta, stock, imagen FROM producto WHERE (nombre LIKE ? OR codigo_barras LIKE ? ) LIMIT ? OFFSET ? "
...
exports.SELECT_SALES = "SELECT v.id, v.subtotal, v.impuesto_generado_venta, v.descuento, v.total, u.nombre_usuario, c.nombre AS cliente, vch.id as voucher_id, v.fecha FROM venta v INNER JOIN usuarios u ON u.id = v.usuario_id INNER JOIN cliente c ON c.id = v.cliente_id INNER JOIN voucher vch ON vch.id = v.voucher_id LIMIT ? OFFSET ?"
exports.SELECT_SEARCH_SALES = "SELECT v.id, v.subtotal, v.impuesto_generado_venta, v.descuento, v.total, u.nombre_usuario, c.nombre AS cliente, vch.id as voucher_id, v.fecha FROM venta v INNER JOIN usuarios u ON u.id = v.usuario_id INNER JOIN cliente c ON c.id = v.cliente_id INNER JOIN voucher vch ON vch.id = v.voucher_id WHERE (v.id LIKE ? OR c.nombre LIKE ?)  LIMIT ? OFFSET ?"
